...
SHIMS=$HERE/shims
FILTER=${1:-}
ROOT=$(mktemp -d "${TMPDIR:-/tmp}/immich-rollback-tests.XXXXXX")
...
mk_legacy() {
  local name=$1 policy=$2 image=ghcr.io/immich-app/immich-server:v2.7.5 d
  ...
  printf 'imgid-immich' >"$d/image_id"
  printf 'imgid-immich' >"$SHIM_STATE/image-ids/${image//[\/:@]/_}"
  printf 'bridge' >"$d/netmode"
  printf 'false' >"$d/autoremove"
  printf 'immich' >"$d/project"
  printf '%s' "$name" >"$d/service"
  printf '4096' >"$d/sizerw"
  printf 'bind||/srv/immich/library|/data|true|rprivate\n' >"$d/mounts"
  printf 'immich_default|%s cid-%s |10.89.0.7|aa:bb:cc:dd:ee:01\n' "$name" "$name" >"$d/networks"
  printf '2283/tcp|0.0.0.0:2283 \n' >"$d/ports"
  printf 'io.podman.compose.project=immich\n' >"$d/labels"
  : >"$d/label"
  printf '%s\0' /usr/bin/podman run "--name=$name" -d --rm --replace \
    --cidfile "/run/user/1000/$name.cid" --label io.podman.compose.project=immich \
    -v /srv/immich/library:/data --net immich_default -e TZ=Asia/Taipei \
    ghcr.io/immich-app/immich-server:v2.7.5 >"$d/createcommand.argv0"
}
...
t_disabled_restart_unit_keeps_the_rename_path() {
  mk_legacy immich_postgres unless-stopped
  mk_legacy immich_server unless-stopped
  eq "$(ql_rollback_strategy immich_postgres immich_server 2>/dev/null)" rename "strategy on a toypark-like host"
  expect_ok app_legacy_retire rename 20260914 "$T/bk" immich_postgres immich_server
  has "$OUT" "renamed immich_server -> immich_server-legacy-20260914"
  eq "$(ncalls 'podman rename immich_postgres immich_postgres-legacy-20260914')" 1 "rename of the db"
  eq "$(ncalls 'podman rename immich_server immich_server-legacy-20260914')" 1 "rename of immich_server"
  ...
  podman container exists immich_server-legacy-20260914 || die_t "the renamed container is missing"
  # and the rollback renames them straight back
  expect_ok app_legacy_restore 20260914 "$T/bk" immich_postgres immich_server
  has "$OUT" "renamed immich_server-legacy-20260914 -> immich_server"
  podman container exists immich_server || die_t "the rollback did not bring immich_server back"
  eq "$(ncalls 'podman create')" 0 "a renamed container is not recreated"
}

t_always_policy_with_a_disabled_unit_is_still_rename() {
  mk_legacy immich_server always
  mk_legacy immich_postgres always
  eq "$(ql_rollback_strategy immich_postgres immich_server 2>/dev/null)" rename "a disabled unit never revives anything"
}
...
t_enabled_restart_unit_and_always_policy_takes_the_capture_path() {
  ...
  mk_legacy immich_postgres always
  mk_legacy immich_server always
  eq "$(ql_rollback_strategy immich_postgres immich_server 2>/dev/null)" capture "strategy on an openclaw-like host"
  expect_ok app_legacy_capture "$T/bk" immich_postgres immich_server
  for c in immich_server immich_postgres; do
  ...
  expect_ok app_legacy_retire capture 20260914 "$T/bk" immich_postgres immich_server
  has "$OUT" "removed immich_server;"
  eq "$(ncalls 'podman rename')" 0 "the capture path must not rename"
  podman container exists immich_server && die_t "immich_server was not removed"
  podman container exists immich_server-legacy-20260914 && die_t "the capture path must not leave a renamed copy"
  return 0
}
...
t_the_capture_path_never_removes_the_anonymous_volumes() {
  enable_restart_unit
  mk_legacy immich_server always
  expect_ok app_legacy_capture "$T/bk" immich_server
  expect_ok app_legacy_retire capture 20260914 "$T/bk" immich_server
...
t_the_rollback_recreates_a_captured_container_with_its_policy() {
  ...
  expect_ok app_legacy_capture "$T/bk" immich_server
  expect_ok app_legacy_retire capture 20260914 "$T/bk" immich_server
  expect_ok app_legacy_restore 20260914 "$T/bk" immich_server
  has "$OUT" "recreated immich_server"
  podman container exists immich_server || die_t "the rollback did not recreate immich_server"
  eq "$(ql_container_restart_policy immich_server)" always "the original restart policy comes back"
}

t_capture_refuses_a_container_the_library_cannot_replay() {
  enable_restart_unit
  mk_api_created immich_server always
  expect_fail app_legacy_capture "$T/bk" immich_server
  has "$OUT" "podman API"
...
t_retire_refuses_to_remove_without_a_capture() {
  enable_restart_unit
  mk_legacy immich_server always
  expect_fail app_legacy_retire capture 20260914 "$T/bk" immich_server
  has "$OUT" "no rollback copy of immich_server"
...
t_capture_is_idempotent_between_prepare_only_and_the_cutover() {
  enable_restart_unit
  mk_legacy immich_server always
  expect_ok app_legacy_capture "$T/bk" immich_server # --prepare-only
  expect_ok app_legacy_capture "$T/bk" immich_server # the cutover reuses the same backup dir
  has "$OUT" "already in"
}
...
t_the_capture_path_works_with_an_empty_suffix() {
  ...
  mk_legacy immich_server always
  expect_ok app_legacy_capture "$T/bk" immich_server
  expect_ok app_legacy_retire capture "" "$T/bk" immich_server
  podman container exists immich_server && die_t "immich_server was not removed"
  expect_ok app_legacy_restore "" "$T/bk" immich_server
  has "$OUT" "recreated immich_server"
  eq "$(ql_container_restart_policy immich_server)" always "the original restart policy comes back"
  # and with no capture either, the refusal names only what could exist
  expect_fail app_legacy_restore "" "$T/empty" immich_server
